...
系统现已支持**波动率自适应**的智能卖出策略，根据不同币种特性自动调整交易参数：

**1. 波动率自动检测**
- 自动识别币种特性（STABLE/MODERATE/HIGH）
- 基于日波动率、周波动率、ATR、极端波动等多维度分析
- 实时监测市场波动特征

**2. 分级卖出策略**
- **STABLE型（BTC/ETH）**：宽松止盈，追求大趋势
  - 止盈阈值：100%
  - 适合长期持有稳定币种

- **MODERATE型（主流币）**：中等参数平衡
  - 止盈阈值：50%
  - 适合常规主流币种
...
**3. 智能卖出机制**
- **快速锁定**：盈利25%立即卖出40%仓位（HIGH型）
- **分段止盈**：不一次性清仓，保留上涨空间
- **动态阈值**：根据当前盈亏自动调整卖出条件
- **自适应止损**：根据杠杆倍数调整止损线（高杠杆更严格）
...
详见完整设计文档：`docs/plans/2026-03-02-adaptive-exit-strategy-design.md`
